...
public typealias TCApiResult = (_ data: Data?, _ error: Error?) -> Void

public enum TCApiResponse {
    case success(Int)
...
extension TCApiRequest {
    public func fetch(with queryItems: [URLQueryItem], completion: @escaping TCApiResult) {
...
extension TCApiRequest {
    public func fetch(with request: URLRequest, completion: @escaping TCApiResult) {
        dataTask = urlSession.dataTask(with: request) { [weak self] data, response, error in
            ...
            let response: TCApiResponse = TCApiResponse(from: response)
            switch response {
            case .success:
                completion(data, nil)
            case .retryRequired:
    ...
    private func retry(of request: URLRequest, error: Error?, completion: @escaping TCApiResult) {
